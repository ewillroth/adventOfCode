const input = `dcbgefa cebd bfega eadbf db cdfaeb dba bfcgda egadcf aedcf | egadcfb eafcd db debc
...
const testOutputs = testLines.map((line) => {
	return line.split(' | ')[1];
});
...
const doTheThing = (line) => {
	...
	// zero, six, and nine are sixDigits
	// two, three, and five are fiveDigits

	// a is a letter that's included in a threeDigit but not in a twoDigit
	seven.split('').forEach((char) => {
		if (!one.includes(char)) letterMap['a'] = char;
	});

	// three is the only fiveDigit that has both digits from one
	fiveDigits.forEach((word) => {
		if (word.includes(one.split('')[0]) && word.includes(one.split('')[1]))
			three = word;
	});

	// b is the letter in four that's not in three
	four.split('').forEach((letter) => {
		if (!three.includes(letter)) letterMap['b'] = letter;
	});

	// add together four and a (abcdf)
	const fourPlusA = four + letterMap['a'];
	// one of the six digits has all of those plus one more
	// the "plus one more" is g
	sixDigits.forEach((word) => {
		if (
			fourPlusA.split('').filter((letter) => word.includes(letter))
				.length === 5
		) {
			const extraLetter = word
				.split('')
				.filter((letter) => !fourPlusA.split('').includes(letter));
			letterMap['g'] = extraLetter[0];
		}
	});

	// add together one and b (cfb)
	// four has one extra letter (d)
	const onePlusB = one + letterMap['b'];
	four.split('').forEach((letter) => {
		if (!onePlusB.includes(letter)) letterMap['d'] = letter;
	});

	// add together three and b
	// eight has one extra letter (e)
	const threePlusB = three + letterMap['b'];
	eight.split('').forEach((letter) => {
		if (!threePlusB.includes(letter)) letterMap['e'] = letter;
	});

	// the six digit number that does not include c and f (letters from one) is 6;
	// c is the letter missing
	sixDigits.forEach((word) => {
		if (
			word.includes(one.split('')[0]) &&
			word.includes(one.split('')[1])
		) {
			//
		} else {
			six = word;
			const c = ['a', 'b', 'c', 'd', 'e', 'f', 'g'].filter((letter) => {
	...
	// f is the last letter left :D
	const f = ['a', 'b', 'c', 'd', 'e', 'f', 'g'].filter((letter) => {
		if (Object.values(letterMap).includes(letter)) return false;
		return true;
	});
	letterMap['f'] = f[0];
	...
	const findNumber = (string) => {
		if (
			string.includes(letterMap['a']) &&
			string.includes(letterMap['b']) &&
			string.includes(letterMap['c']) &&
			string.includes(letterMap['e']) &&
			string.includes(letterMap['f']) &&
			string.includes(letterMap['g']) &&
			!string.includes(letterMap['d'])
		) {
			return '0';
	...
	const scrambledEncodedNumbers = output.split(' ');
	const decodedNumbers = scrambledEncodedNumbers.map((string) => {
		return findNumber(string);
	});
	console.log(scrambledEncodedNumbers);
	console.log(decodedNumbers);
	console.log(partTwoAnswer);
	partTwoAnswer += +decodedNumbers.join('');
};
...
lines.forEach((line) => doTheThing(line));
console.log(partTwoAnswer);
